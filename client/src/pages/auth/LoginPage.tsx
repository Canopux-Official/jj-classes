      ...
      <Box sx={loginStyles.rightSection}>
        <Box sx={loginStyles.formBox}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 5 }}>
            {step === 'PHONE' ? 'Login with your registered mobile number' : `Enter the 4-digit OTP sent to +91 ${phone}`}
          </Typography>
          ...
          <AnimatePresence mode='wait'>
            {step === 'PHONE' ? (
              <motion.div key="phone-step" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                ...
                />
                <Button fullWidth variant="contained" size="large" onClick={handleSendOtp} disabled={loading} sx={loginStyles.actionBtn}>
              ...
              <motion.div key="otp-step" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <Typography onClick={() => setStep('PHONE')} sx={{ cursor: 'pointer', color: 'text.secondary', fontSize: '0.85rem', textDecoration: 'underline' }}>
